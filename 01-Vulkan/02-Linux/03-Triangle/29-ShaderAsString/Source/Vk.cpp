...
int main(void)
{
    ...
    //* Set Window Caption
    XStoreName(gpDisplay, window, "Atharv Natu : Vulkan Triangle With Shaders As String");
...
VkResult createShaders(void)
{
    ...
    bSpvDone = glsl2Spv(VK_SHADER_STAGE_VERTEX_BIT, vertexShaderSourceCode, vertex_spv);
    if (bSpvDone == False)
    {
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
    ...
    bSpvDone = glsl2Spv(VK_SHADER_STAGE_FRAGMENT_BIT, fragmentShaderSourceCode, fragment_spv);
    if (bSpvDone == False)
    {
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
