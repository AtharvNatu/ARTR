...
-(VkResult) initialize
{
    ...
    //! Load Mesh
    NSString *modelFilePath = [[NSBundle mainBundle] pathForResource:@"Monkey-Head" ofType:@"obj.model"];
    const char* modelFileName = [modelFilePath UTF8String];
    int ret = loadMesh(modelFileName);
    if (ret == FAILURE)
    {
        printf("%s() => loadMesh() Failed : %d !!!\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return vkResult;
    }
    else
        printf("%s() => loadMesh() Succeeded\n", __func__);
    ...
    //! Create Pipeline
    vkResult = [self createPipeline:&vkPipeline_lineMode :VK_POLYGON_MODE_LINE];
    if (vkResult != VK_SUCCESS)
    {
        printf("%s() => createPipeline() Failed For vkPipeline_lineMode: %d !!!\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return vkResult;
    }
    else
        printf("%s() => createPipeline() Succeeded For vkPipeline_lineMode\n", __func__);

    vkResult = [self createPipeline:&vkPipeline_fillMode :VK_POLYGON_MODE_FILL];
    if (vkResult != VK_SUCCESS)
    {
        printf("%s() => createPipeline() Failed For vkPipeline_fillMode: %d !!!\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return vkResult;
    }
    else
        printf("%s() => createPipeline() Succeeded For vkPipeline_fillMode\n", __func__);
...
-(void) uninitialize
{
    ...
    if (vkPipeline_fillMode)
    {
        vkDestroyPipeline(vkDevice, vkPipeline_fillMode, NULL);
        vkPipeline_fillMode = VK_NULL_HANDLE;
        printf("%s() => vkDestroyPipeline() Succeeded For vkPipeline_fillMode\n", __func__);
    }
    ...
    if (vkPipeline_lineMode)
    {
        vkDestroyPipeline(vkDevice, vkPipeline_lineMode, NULL);
        vkPipeline_lineMode = VK_NULL_HANDLE;
        printf("%s() => vkDestroyPipeline() Succeeded For vkPipeline_lineMode\n", __func__);
    }
...
-(VkResult) createVertexBuffer
{
    ...
    {
        vkResult = VK_ERROR_OUT_OF_DEVICE_MEMORY;
        printf("%s() => Host Visible Memory Not Found For Vertex Buffer: %d !!!\n", __func__, vkResult);
        return vkResult;
    }
...
-(VkResult) buildCommandBuffers
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBeginRenderPass(vkCommandBuffer_array[i], &vkRenderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
        {
            ...
            vkCmdDrawIndexed(
                vkCommandBuffer_array[i],
                (unsigned int)gpVertexIndices->size,
