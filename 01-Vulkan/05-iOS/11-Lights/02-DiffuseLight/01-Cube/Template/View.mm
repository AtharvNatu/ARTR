...
-(void) uninitialize
{
    ...
    if (vertexData_normal.vkDeviceMemory)
    {
        ...
        printf("%s() => vkFreeMemory() Succeeded For vertexData_normal.vkDeviceMemory\n", __func__);
    }
    ...
    if (vertexData_normal.vkBuffer)
    {
        ...
        printf("%s() => vkDestroyBuffer() Succeeded For vertexData_normal.vkBuffer\n", __func__);
    }
...
-(VkResult) createVertexBuffer
{
    ...
    {
        vkResult = VK_ERROR_OUT_OF_DEVICE_MEMORY;
        printf("%s() => Host Visible Memory Not Found For Position Buffer : %d !!!\n", __func__, vkResult);
        return vkResult;
    }
    ...
    //* Step - 8.1
    VkBool32 foundMatchingMemoryType_vertex_normal = VK_FALSE;
    for (uint32_t i = 0; i < vkPhysicalDeviceMemoryProperties.memoryTypeCount; i++)
    {
        //* Step - 8.2
        if ((vkMemoryRequirements.memoryTypeBits & 1) == 1)
        {
            ...
            if (vkPhysicalDeviceMemoryProperties.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
            {
                //* Step - 8.4
                vkMemoryAllocateInfo.memoryTypeIndex = i;
                foundMatchingMemoryType_vertex_normal = VK_TRUE;
                break;
            }
        }
    ...
    //* Check For memoryTypeIndex != 0 On MoltenVK
    if (foundMatchingMemoryType_vertex_normal == VK_FALSE)
    {
        vkResult = VK_ERROR_OUT_OF_DEVICE_MEMORY;
    ...
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkMapMemory() Failed For Vertex NormalsBuffer : %d !!!\n", __func__, vkResult);
    else
