...
-(void) uninitialize
{
    ...
    if (vertexData_position_index.vkDeviceMemory)
    {
        ...
        printf("%s() => vkFreeMemory() Succeeded For vertexData_position_index.vkDeviceMemory\n", __func__);
    }
    ...
    if (vertexData_position_index.vkBuffer)
    {
        ...
        printf("%s() => vkDestroyBuffer() Succeeded For vertexData_position_index.vkBuffer\n", __func__);
    }
...
-(VkResult) createVertexBuffer
{
    ...
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkCreateBuffer() Failed For Position Index Buffer : %d !!!\n", __func__, vkResult);
    else
        printf("%s() => vkCreateBuffer() Succeeded For Position Index Buffer\n", __func__);
    ...
    {
        vkResult = VK_ERROR_OUT_OF_DEVICE_MEMORY;
        printf("%s() => Host Visible Memory Not Found For Position Index Buffer : %d !!!\n", __func__, vkResult);
        return vkResult;
    }
    ...
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkAllocateMemory() Failed For Position Index Buffer : %d !!!\n", __func__, vkResult);
    else
        printf("%s() => vkAllocateMemory() Succeeded For Position Index Buffer\n", __func__);

    //* Step - 10
    //! Binds Vulkan Device Memory Object Handle with the Vulkan Buffer Object Handle
    vkResult = vkBindBufferMemory(vkDevice, vertexData_position_index.vkBuffer, vertexData_position_index.vkDeviceMemory, 0);
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkBindBufferMemory() Failed For Position Index Buffer : %d !!!\n", __func__, vkResult);
    else
        printf("%s() => vkBindBufferMemory() Succeeded For Position Index Buffer\n", __func__);

    //* Step - 11
    data = NULL;
    vkResult = vkMapMemory(vkDevice, vertexData_position_index.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkMapMemory() Failed For Position Index Buffer : %d !!!\n", __func__, vkResult);
    else
        printf("%s() => vkMapMemory() Succeeded For Position Index Buffer\n", __func__);
