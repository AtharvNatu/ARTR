...
-(void) uninitialize
{
    ...
    if (vertexData_color.vkDeviceMemory)
    {
        ...
        printf("%s() => vkFreeMemory() Succeeded For vertexData_color.vkDeviceMemory\n", __func__);
    }
    ...
    if (vertexData_color.vkBuffer)
    {
        ...
        printf("%s() => vkDestroyBuffer() Succeeded For vertexData_color.vkBuffer\n", __func__);
    }
...
-(VkResult) createVertexBuffer
{
    ...
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkCreateBuffer() Failed For Vertex Color Buffer : %d !!!\n", __func__, vkResult);
    else
        printf("%s() => vkCreateBuffer() Succeeded For Vertex Color Buffer\n", __func__);
    ...
    //* Step - 8.1
    VkBool32 foundMatchingMemoryType_vertex_color = VK_FALSE;
    for (uint32_t i = 0; i < vkPhysicalDeviceMemoryProperties.memoryTypeCount; i++)
    {
        //* Step - 8.2
        if ((vkMemoryRequirements.memoryTypeBits & 1) == 1)
        {
            ...
            if (vkPhysicalDeviceMemoryProperties.memoryTypes[i].propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT)
            {
                //* Step - 8.4
                vkMemoryAllocateInfo.memoryTypeIndex = i;
                foundMatchingMemoryType_vertex_color = VK_TRUE;
                break;
            }
        }
    ...
    //* Check For memoryTypeIndex != 0 On MoltenVK
    if (foundMatchingMemoryType_vertex_color == VK_FALSE)
    {
        vkResult = VK_ERROR_OUT_OF_DEVICE_MEMORY;
        printf("%s() => Host Visible Memory Not Found For Vertex Color Buffer : %d !!!\n", __func__, vkResult);
        return vkResult;
    }
    ...
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkAllocateMemory() Failed For Vertex Color Buffer : %d !!!\n", __func__, vkResult);
    else
        printf("%s() => vkAllocateMemory() Succeeded For Vertex Color Buffer\n", __func__);

    //* Step - 10
    //! Binds Vulkan Device Memory Object Handle with the Vulkan Buffer Object Handle
    vkResult = vkBindBufferMemory(vkDevice, vertexData_color.vkBuffer, vertexData_color.vkDeviceMemory, 0);
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkBindBufferMemory() Failed For Vertex Color Buffer : %d !!!\n", __func__, vkResult);
    else
        printf("%s() => vkBindBufferMemory() Succeeded For Vertex Color Buffer\n", __func__);

    //* Step - 11
    data = NULL;
    vkResult = vkMapMemory(vkDevice, vertexData_color.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
    if (vkResult != VK_SUCCESS)
        printf("%s() => vkMapMemory() Failed For Vertex Color Buffer : %d !!!\n", __func__, vkResult);
    else
        printf("%s() => vkMapMemory() Succeeded For Vertex Color Buffer\n", __func__);
