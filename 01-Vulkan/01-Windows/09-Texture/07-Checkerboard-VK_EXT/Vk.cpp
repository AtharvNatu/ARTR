...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
    ...
    hwnd = CreateWindowEx(
        WS_EX_APPWINDOW,
        szAppName,
        TEXT("Atharv Natu : Vulkan Procedural Checkerboard Texture"),
...
VkResult initialize(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        fprintf(gpFile, "%s() => createTexture() Failed For Checkerboard : %d !!!\n", __func__, vkResult);
        return vkResult;
    }
    else
        fprintf(gpFile, "%s() => createTexture() Succeeded For Checkerboard\n", __func__);
...
VkResult fillInstanceExtensionNames(void)
{
    ...
    VkBool32 physicalDeviceProperties2ExtensionFound = VK_FALSE;
    ...
    for (uint32_t i = 0; i < instanceExtensionCount; i++)
    {
        ...
        {
            physicalDeviceProperties2ExtensionFound = VK_TRUE;
    ...
    if (physicalDeviceProperties2ExtensionFound = VK_FALSE)
    {
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        fprintf(gpFile, "%s() => VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME Extension Not Found !!!\n", __func__);
        return vkResult;
    }
    else
        fprintf(gpFile, "%s() => VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME Extension Found\n", __func__);
...
VkResult fillDeviceExtensionNames(void)
{
    ...
    VkBool32 vertexDynamicStateExtensionFound = VK_FALSE;
    ...
    for (uint32_t i = 0; i < deviceExtensionCount; i++)
    {
        ...
        if (strcmp(deviceExtensionNames_array[i], VK_EXT_VERTEX_INPUT_DYNAMIC_STATE_EXTENSION_NAME) == 0)
        {
            vertexDynamicStateExtensionFound = VK_TRUE;
            enabledDeviceExtensionNames_array[enabledDeviceExtensionCount++] = VK_EXT_VERTEX_INPUT_DYNAMIC_STATE_EXTENSION_NAME;
        }
    }
...
VkResult createVulkanDevice(void)
{
    ...
    VkResult fillDeviceExtensionNames(void);
    VkResult createVertexInputCallbackFunction(void);
...
VkResult createVertexInputCallbackFunction(void)
{
    ...
    // Code
    vkCmdSetVertexInputEXT_fnptr = (PFN_vkCmdSetVertexInputEXT)vkGetDeviceProcAddr(vkDevice, "vkCmdSetVertexInputEXT");
    if (vkCmdSetVertexInputEXT_fnptr == NULL)
    {
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        fprintf(gpFile, "%s() => vkGetDeviceProcAddr() Failed To Get Function Pointer For vkCmdSetVertexInputEXT_fnptr !!!\n", __func__);
        return vkResult;
    }
    else
        fprintf(gpFile, "%s() => vkGetDeviceProcAddr() Succeeded To Get Function Pointer For vkCmdSetVertexInputEXT_fnptr\n", __func__);
...
VkResult createDescriptorSet(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        ...
        fprintf(gpFile, "%s() => vkAllocateDescriptorSets() Succeeded For vkDescriptorSet_straight\n", __func__);
...
VkResult createPipeline(void)
{
    ...
    vkVertexInputAttributeDescription2EXT_array[1].offset = 0;
    ...
    //! Dynamic State !//
    VkDynamicState vkDynamicState = VK_DYNAMIC_STATE_VERTEX_INPUT_EXT;
    ...
    vkPipelineDynamicStateCreateInfo.pDynamicStates = &vkDynamicState;
...
VkResult buildCommandBuffers(void)
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBeginRenderPass(vkCommandBuffer_array[i], &vkRenderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
        {
            ...
            vkCmdSetVertexInputEXT_fnptr(
                vkCommandBuffer_array[i], 
                2, 
                vkVertexInputBindingDescription2EXT_array, 
                2,
                vkVertexInputAttributeDescription2EXT_array
            );
