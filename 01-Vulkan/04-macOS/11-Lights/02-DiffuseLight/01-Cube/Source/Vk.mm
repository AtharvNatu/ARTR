...
-(void) applicationDidFinishLaunching:(NSNotification *)notification 
{
    ...
    [window setTitle:@"Atharv Natu : macOS : Vulkan Diffuse Light On Cube"];
...
-(VkResult) createVertexBuffer
{
    ...
    {
        vkResult = VK_ERROR_OUT_OF_DEVICE_MEMORY;
        fprintf(gpFile, "%s() => Host Visible Memory Not Found For Position Buffer : %d !!!\n", __func__, vkResult);
        return vkResult;
    }
    ...
    if (vkResult != VK_SUCCESS)
        fprintf(gpFile, "%s() => vkMapMemory() Failed For Vertex NormalsBuffer : %d !!!\n", __func__, vkResult);
    else
