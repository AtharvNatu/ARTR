...
if (( SPV == 1)); then
    ...
    echo "--------------------------------------------------------------------------------"
    cd Shaders
        glslangValidator -V -H -o Teapot.vert.spv Teapot.vert
        glslangValidator -V -H -o Teapot.frag.spv Teapot.frag
        glslangValidator -V -H -o Cube.vert.spv Cube.vert
        glslangValidator -V -H -o Cube.frag.spv Cube.frag
        if [ $? -ne 0 ]; then
            echo "Shader Compilation Failed !!!"
            exit 1
        fi
        mv Teapot.vert.spv ../"$BIN_DIR"
        mv Teapot.frag.spv ../"$BIN_DIR"
        mv Cube.vert.spv ../"$BIN_DIR"
        mv Cube.frag.spv ../"$BIN_DIR"
    cd ..
fi
