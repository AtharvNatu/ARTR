    ...
    while (1)
    {
        ...
        if (bPendingSingleTap)
        {
            ...
            if (now - pendingSingleTapTime > 300)
            {
                ...
                // Single Tap Count For Tweaked Smiley
                singleTapCount++;
                if (singleTapCount > 4)
                    singleTapCount = 0;
            }
...
VkResult initialize(void)
{
    ...
    if (vkResult != VK_SUCCESS)
    {
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => createTexture() Failed For Smiley.png : %d !!!\n", __func__, vkResult);
        return vkResult;
    }
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => createTexture() Succeeded For Smiley.png\n", __func__);
...
VkResult updateUniformBuffer(void)
{
    ...
    host_uniformData.modelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, -3.0f));
