...
VkResult uninitialize(void)
{
    ...
    if (vkShaderModule_tessellation_evaluation_shader)
    {
        ...
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkDestroyShaderModule() Succeeded For Tessellation Evaluation Shader\n", __func__);
    }
    ...
    if (vkShaderModule_tessellation_control_shader)
    {
        ...
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkDestroyShaderModule() Succeeded For Tessellation Control Shader\n", __func__);
    }
...
VkResult createVulkanDevice(void)
{
    ...
    else
    {
        ...
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "ERROR : Selected Device Does Not Support Tessellation Shader : %d !!!\n", vkResult);
        return VK_ERROR_INITIALIZATION_FAILED;
    }
...
VkResult updateUniformBuffer(void)
{
    ...
    host_UniformData.numberOfLineSegments.x = uiNumberOfLineSegments;
    
    //* Update Number of Strips
    host_UniformData.numberOfStrips.x = (float)1.0;

    //* Update Line Color
    host_UniformData.lineColor = glm::vec4(1.0, 1.0, 0.0, 1.0); // Yellow Color

    switch(uiNumberOfLineSegments)
    {
        case 1:
            host_UniformData.lineColor = glm::vec4(1.0, 0.0, 0.0, 1.0);
        break;

        case 30:
            host_UniformData.lineColor = glm::vec4(0.0, 1.0, 0.0, 1.0);
        break;
    }
...
VkResult createShaders(void)
{
    ...
    if (vkResult != VK_SUCCESS)
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkCreateShaderModule() Failed For Tessellation Control Shader : %d !!!\n", __func__, vkResult);
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkCreateShaderModule() Succeeded For Tessellation Control Shader\n", __func__);
    ...
    __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => Tessellation Control Shader Module Successfully Created\n", __func__);
    //! ---------------------------------------------------------------------------------------------------------------------------
    ...
    if (vkResult != VK_SUCCESS)
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkCreateShaderModule() Failed For Tessellation Evaluation Shader : %d !!!\n", __func__, vkResult);
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkCreateShaderModule() Succeeded For Tessellation Evaluation Shader\n", __func__);
    ...
    __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => Tessellation Evaluation Shader Module Successfully Created\n", __func__);
    //! ---------------------------------------------------------------------------------------------------------------------------
...
VkResult createPipeline(void)
{
    ...
    vkVertexInputBindingDescription_array[0].stride = sizeof(float) * 2; // As the geometry is line, and does not have z
