                ...
                case AMOTION_EVENT_ACTION_DOWN:
                {
                    ...
                    if (bPendingSingleTap)
                    {
                        ...
                        if (timeSinceLastTap < 300 && distance < 100.0f)
                        {
                            ...
                            bPolygonLineMode = !bPolygonLineMode;
                            buildCommandBuffers();
                        }    
                        else
                        {
...
VkResult initialize(void)
{
    ...
    VkResult createPipeline(VkPipeline*, VkPolygonMode);
    ...
    //! Create Pipeline
    vkResult = createPipeline(&vkPipeline_lineMode, VK_POLYGON_MODE_LINE);
    if (vkResult != VK_SUCCESS)
    {
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => createPipeline() Failed For vkPipeline_lineMode : %d !!!\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return vkResult;
    }
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => createPipeline() Succeeded For vkPipeline_lineMode\n", __func__);

    vkResult = createPipeline(&vkPipeline_fillMode, VK_POLYGON_MODE_FILL);
    if (vkResult != VK_SUCCESS)
    {
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => createPipeline() Failed For vkPipeline_fillMode: %d !!!\n", __func__, vkResult);
        vkResult = VK_ERROR_INITIALIZATION_FAILED;
        return vkResult;
    }
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => createPipeline() Succeeded For vkPipeline_fillMode\n", __func__);
...
VkResult resize(int width, int height)
{
    ...
    bInitialized = false;
    {
        ...
        //* Destroy Pipeline
        if (vkPipeline_fillMode)
        {
            vkDestroyPipeline(vkDevice, vkPipeline_fillMode, NULL);
            vkPipeline_fillMode = VK_NULL_HANDLE;
        }

        if (vkPipeline_lineMode)
        {
            vkDestroyPipeline(vkDevice, vkPipeline_lineMode, NULL);
            vkPipeline_lineMode = VK_NULL_HANDLE;
        }
...
VkResult uninitialize(void)
{
    ...
    if (vkPipeline_fillMode)
    {
        vkDestroyPipeline(vkDevice, vkPipeline_fillMode, NULL);
        vkPipeline_fillMode = VK_NULL_HANDLE;
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkDestroyPipeline() Succeeded For vkPipeline_fillMode\n", __func__);
    }
    ...
    if (vkPipeline_lineMode)
    {
        vkDestroyPipeline(vkDevice, vkPipeline_lineMode, NULL);
        vkPipeline_lineMode = VK_NULL_HANDLE;
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkDestroyPipeline() Succeeded For vkPipeline_lineMode\n", __func__);
    }
    ...
    if (vertexData_index.vkDeviceMemory)
    {
        ...
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkFreeMemory() Succeeded For vertexData_index.vkDeviceMemory\n", __func__);
    }
    ...
    if (vertexData_index.vkBuffer)
    {
        ...
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkDestroyBuffer() Succeeded For vertexData_index.vkBuffer\n", __func__);
    }
...
VkResult getPhysicalDevice(void)
{
    ...
    {
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => Selected Physical Device Supports Non-Solid Fill Mode\n", __func__);
...
VkResult createVertexBuffer(void)
{
    ...
    vkBufferCreateInfo.size = (gpVertex->size) * sizeof(float);
    vkBufferCreateInfo.usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT;
    
    //* Step - 6
    vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vertexData_position.vkBuffer);
    if (vkResult != VK_SUCCESS)
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkCreateBuffer() Failed For Vertex Buffer : %d !!!\n", __func__, vkResult);
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkCreateBuffer() Succeeded For Vertex Buffer\n", __func__);
    ...
    if (vkResult != VK_SUCCESS)
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkAllocateMemory() Failed For Vertex Buffer : %d !!!\n", __func__, vkResult);
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkAllocateMemory() Succeeded For Vertex Buffer\n", __func__);
    ...
    if (vkResult != VK_SUCCESS)
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkBindBufferMemory() Failed For Vertex Buffer : %d !!!\n", __func__, vkResult);
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkBindBufferMemory() Succeeded For Vertex Buffer\n", __func__);

    //* Step - 11
    void* data = NULL;
    vkResult = vkMapMemory(vkDevice, vertexData_position.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
    if (vkResult != VK_SUCCESS)
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkMapMemory() Failed For Vertex Buffer : %d !!!\n", __func__, vkResult);
    else
        __android_log_print(ANDROID_LOG_INFO, "ADN:", "%s() => vkMapMemory() Succeeded For Vertex Buffer\n", __func__);

    //* Step - 12
    memcpy(data, gpVertex->ptr, (gpVertex->size) * sizeof(float));
    ...
    vkBufferCreateInfo.flags = 0;   //! Valid Flags are used in sparse(scattered) buffers
    vkBufferCreateInfo.pNext = NULL;
    vkBufferCreateInfo.size = (gpVertexIndices->size) * sizeof(int);
    ...
    //* Step - 12
    memcpy(data, gpVertexIndices->ptr, (gpVertexIndices->size) * sizeof(int));
...
VkResult createPipeline(VkPipeline* pPipeline, VkPolygonMode polygonMode)
{
    // Variable Declarations
    VkResult vkResult = VK_SUCCESS;
...
VkResult buildCommandBuffers(void)
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++)
    {
        ...
        vkCmdBeginRenderPass(vkCommandBuffer_array[i], &vkRenderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
        {
            //! Bind with Pipeline
            if (bPolygonLineMode)
                vkCmdBindPipeline(vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_GRAPHICS, vkPipeline_lineMode);
            else
                vkCmdBindPipeline(vkCommandBuffer_array[i], VK_PIPELINE_BIND_POINT_GRAPHICS, vkPipeline_fillMode);
