...
struct VertexOutput
{
    ...
    @location(3) lightDirection2: vec3<f32>,
...
fn main(output:VertexOutput) -> @location(0) vec4<f32>
{
    ...
    {
        var ambient : array<vec3<f32>, 3>;
        var diffuse : array<vec3<f32>, 3>;
        var specular : array<vec3<f32>, 3>;
        var normalized_light_direction : array<vec3<f32>, 3>;
        var reflection_vector : array<vec3<f32>, 3>;
        ...
        let lightDirection : array<vec3<f32>, 3> = array<vec3<f32>, 3>(output.lightDirection0, output.lightDirection1, output.lightDirection2);
