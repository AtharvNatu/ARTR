...
async function initialize()
{
    ...
    //* Mesh
    objMesh = await loadMesh("../Assets/Models/Monkey-Head.obj.model");
    if (objMesh == null)
    {
        console.log("Failed To Load Mesh !!!");
        throw Error("Failed To Load Mesh !!!");
    }
    else
        console.log("Mesh Successfully Loaded");

    //* Vertex Buffer
    vertex_buffer = createVertexBuffer(objMesh.vertices);
    if (vertex_buffer == null)
    {
        console.log("Failed To Create Interleaved Vertex Buffer !!!");
        throw Error("Failed To Create Interleaved Vertex Buffer !!!");
    }
    else
        console.log("Interleaved Vertex Buffer Successfully Created");

    //* Index Buffer
    index_buffer = createIndexBuffer(objMesh.indices);
    if (index_buffer == null)
    {
        ...
        console.log("Index Buffer Successfully Created");
    ...
    //! Load Texture
    //! ------------------------------------------------------------------------------------------------------------------
    texture_wood = await loadTexture("../Assets/Images/Wood.png");
    if (texture_wood == null)
    {
        console.log("Failed To Load Wood Texture !!!");
        throw Error("Failed To Load Wood Texture !!!");
    }
    else
    ...
    sampler_wood = device.createSampler(samplerDescriptor);
    if (sampler_wood == null)
    {
    ...
    bindGroup_texture_sampler = createBindGroup_texture_sampler(0, texture_wood, 1, sampler_wood, bindGroupLayout_texture_sampler);
    //! ------------------------------------------------------------------------------------------------------------------
    ...
    {
        shaderLocation: 2,  //* Maps to location(2) in Vertex Shader
...
function display()
{
    ...
    mat4.rotateY(modelViewMatrix, modelViewMatrix, degreeToRadians(angle));
    ...
    const renderPassEncoder = commandEncoder.beginRenderPass(renderPassDescriptor);
    {
        ...
        renderPassEncoder.setVertexBuffer(0, vertex_buffer);
        renderPassEncoder.setIndexBuffer(index_buffer, "uint32");
        renderPassEncoder.setBindGroup(0, bindGroup_mvpUniform);
        renderPassEncoder.setBindGroup(1, bindGroup_texture_sampler);
        renderPassEncoder.drawIndexed(objMesh.indexCount);
    }
